...
export default async function handler(req, res) {
  console.log(
    `[ANALYZE INDEXES] ${req.method} /api/admin/analyze-database-indexes`,
  );
  ...
  try {
    ...
    for (const collectionName of collections) {
      try {
        console.log(
          `[ANALYZE INDEXES] Analyzing ${collectionName} collection...`,
        );
        ...
        let indexUsage = [];
        try {
          indexUsage = await collection
            .aggregate([{ $indexStats: {} }])
            .toArray();
        } catch (indexStatsError) {
          console.log(
            `[ANALYZE INDEXES] Index usage stats not available for ${collectionName}`,
          );
        }
        ...
        analysis[collectionName] = {
          ...
          indexSizeTotal: indexes.reduce(
            (total, idx) => total + (idx.size || 0),
            0,
          ),
        };
      } catch (collectionError) {
        console.log(
          `[ANALYZE INDEXES] Error analyzing ${collectionName}:`,
    ...
    console.log(
      `[ANALYZE INDEXES] ✅ Analysis complete for ${collections.length} collections`,
    );
  ...
  Object.entries(analysis).forEach(([collectionName, data]) => {
    ...
    switch (collectionName) {
      ...
      case "stories":
        if (
          !indexNames.some(
            (name) => name.includes("published") && name.includes("createdAt"),
          )
        ) {
          recommendations.push({
            collection: collectionName,
            ...
            priority: "high",
            message:
              "Missing compound published+createdAt index for story listing queries",
          });
        }
        break;
    ...
    if (
      data.totalDocuments > 10000 &&
      data.indexSizeTotal / data.totalSize < 0.1
    ) {
      recommendations.push({
        collection: collectionName,
        type: "performance",
    ...
    data.indexUsage.forEach((usage) => {
      if (
        usage.name !== "_id_" &&
        (!usage.accesses.ops || usage.accesses.ops === 0)
      ) {
        recommendations.push({
          collection: collectionName,
          type: "unused_index",
