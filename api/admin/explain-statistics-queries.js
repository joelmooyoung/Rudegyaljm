...
export default async function handler(req, res) {
  console.log(
    `[EXPLAIN STATS] ${req.method} /api/admin/explain-statistics-queries`,
  );
  ...
  try {
    ...
    console.log(
      "[EXPLAIN STATS] Starting comprehensive EXPLAIN analysis of statistics queries...",
    );
    ...
    const statisticsQueries = [
      ...
      // === COMMENT COLLECTION QUERIES ===
      {
        ...
        type: "count",
        query: () =>
          db.collection("comments").find({}).explain("executionStats"),
      },
      {
        category: "Comment Analytics",
      ...
      // === RATING COLLECTION QUERIES ===
      {
        ...
        type: "count",
        query: () =>
          db.collection("ratings").find({}).explain("executionStats"),
      },
      {
        category: "Rating Analytics",
    ...
    console.log(
      `[EXPLAIN STATS] Analyzing ${statisticsQueries.length} statistics queries...`,
    );
    ...
    for (const queryDef of statisticsQueries) {
      try {
        ...
        console.log(
          `[EXPLAIN STATS] ✅ ${queryDef.name}: ${executionTime}ms (${analysis.issues.length} issues)`,
        );
      } catch (queryError) {
        console.log(
          `[EXPLAIN STATS] ❌ ${queryDef.name} failed:`,
          queryError.message,
        );
        analysisResults.push({
          ...queryDef,
          ...
          analysis: {
            issues: [
              { type: "error", severity: "high", message: queryError.message },
            ],
          },
        });
        problemQueries++;
    ...
    const recommendations =
      generateOptimizationRecommendations(analysisResults);
    ...
    const criticalIssues = analysisResults.filter(
      (r) => r.analysis && r.analysis.issues.some((i) => i.severity === "high"),
    );

    console.log(
      `[EXPLAIN STATS] ✅ Analysis complete: ${totalQueries} queries, ${problemQueries} with issues`,
    );
...
function analyzeExecutionPlan(explainResult, queryDef) {
  ...
  try {
    ...
    if (executionStats) {
      ...
      analysis.executionTimeEstimate =
        executionStats.executionTimeMillisEstimate || 0;
      ...
      for (const stage of explainResult.stages) {
        if (stage.$match && !stage.$match.inputStage) {
          issues.push({
            ...
            severity: "medium",
            message:
              "$match stage should be first in aggregation pipeline for better performance.",
          });
        }
      }
    }
...
function extractQueryFields(queryDef) {
  ...
  if (
    queryDef.name.includes("createdAt") ||
    queryDef.name.includes("Week") ||
    queryDef.name.includes("Month")
  ) {
    ...
    switch (issueType) {
      ...
      case "slow_query":
        recommendations.push({
          ...
          affectedQueries: issues.map((i) => i.queryName),
          solution:
            "Review query patterns and add appropriate indexes or optimize aggregation pipelines.",
        });
        break;
      ...
      case "inefficient_index":
        recommendations.push({
          ...
          affectedQueries: issues.map((i) => i.queryName),
          solution:
            "Create more selective compound indexes or adjust query patterns.",
        });
        break;
      ...
      case "large_scan":
        recommendations.push({
          ...
          affectedQueries: issues.map((i) => i.queryName),
          solution:
            "Add covering indexes or implement result caching for frequently accessed data.",
        });
        break;
    }
  });
...
function groupResultsByCategory(results) {
  ...
  results.forEach((result) => {
    ...
    grouped[result.category].push({
      ...
      criticalIssues: result.analysis
        ? result.analysis.issues.filter((i) => i.severity === "high").length
        : 0,
    });
  });
