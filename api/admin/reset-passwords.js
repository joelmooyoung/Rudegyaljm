  ...
  try {
    ...
    if (users.length === 0) {
      return res.status(200).json({
        success: true,
        message: "No users found in database to reset passwords.",
        updatedUsers: [],
      });
    }
    ...
    console.log(`[RESET PASSWORDS API] ✅ Successfully updated ${updatedUsers.length} users`);

    return res.status(200).json({
      success: true,
      message: `Successfully reset passwords for ${updatedUsers.length} users`,
