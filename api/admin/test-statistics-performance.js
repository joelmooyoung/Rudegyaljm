...
export default async function handler(req, res) {
  console.log(
    `[STATS PERFORMANCE] ${req.method} /api/admin/test-statistics-performance`,
  );
  ...
  try {
    ...
    console.log(
      "[STATS PERFORMANCE] Starting statistics query performance test...",
    );
      ...
      },
      {
        ...
        operation: async () => {
          return await db
            ...
            .find(
              { published: true },
              { projection: { storyId: 1, title: 1, createdAt: 1 } },
            )
            .sort({ createdAt: -1 })
            .limit(10)
            .toArray();
    ...
    for (const test of testQueries) {
      try {
        ...
        console.log(
          `[STATS PERFORMANCE] ❌ ${test.name} failed:`,
          queryError.message,
        );
        results.push({
          name: test.name,
    ...
    const averageExecutionTime =
      successfulQueries.length > 0 ? totalTime / successfulQueries.length : 0;
    ...
    console.log(
      `[STATS PERFORMANCE] ✅ Performance test complete: ${performanceAnalysis.totalExecutionTime}ms total`,
    );
...
function generatePerformanceRecommendations(results, analysis) {
  ...
  if (analysis.performance === "slow" || analysis.performance === "very_slow") {
    recommendations.push({
      ...
      priority: "high",
      message:
        "Overall query performance is suboptimal. Database index optimization is strongly recommended.",
