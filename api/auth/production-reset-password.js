...
export default async function handler(req, res) {
  ...
  console.log(
    `🔐 [PRODUCTION RESET PASSWORD] Processing token: ${token.substring(0, 20)}...`,
  );
  ...
  try {
    ...
    console.log(
      `🔐 [PRODUCTION RESET PASSWORD] ✅ Password reset successful for: ${email}`,
    );
    ...
    console.error(
      "🔐 [PRODUCTION RESET PASSWORD] Error processing token:",
      error,
    );
    return res.status(400).json({
      success: false,
