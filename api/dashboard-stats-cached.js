...
export default async function handler(req, res) {
  console.log(
    `[DASHBOARD STATS CACHED] ${req.method} /api/dashboard-stats-cached`,
  );
  ...
  if (!forceRefresh) {
    ...
    if (cachedData) {
      ...
      res.setHeader(
        "X-Cache-Source",
        cacheManager.redisClient ? "Redis" : "Memory",
      );
      res.setHeader(
        "Cache-Control",
        `public, max-age=${CACHE_CONFIG.STATS_TTL / 1000}`,
      );
  ...
  try {
    console.log(
      "[DASHBOARD STATS CACHED] Cache miss - generating fresh data...",
    );
    ...
    const dashboardData = {
      ...
      engagement: {
        ...
        engagementRate:
          totalStories > 0
            ? Math.round(((totalComments + totalLikes) / totalStories) * 100) /
              100
            : 0,
      },
      ...
      activity: {
        ...
        loginSuccessRate:
          Math.round((loginSuccessRate[0]?.successRate || 0) * 10) / 10,
      },
    ...
    console.log(
      `[DASHBOARD STATS CACHED] ✅ Fresh data generated and cached:`,
      {
    ...
    if (staleCachedData) {
      console.log(
        "[DASHBOARD STATS CACHED] Returning stale cache due to error",
      );
