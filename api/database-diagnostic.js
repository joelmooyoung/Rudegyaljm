...
export default async function handler(req, res) {
  ...
  try {
    ...
    const storiesCollectionExists = collections.some(
      (c) => c.name === "stories",
    );
    ...
    diagnostic.stories.published = await Story.countDocuments({
      published: true,
    });
    diagnostic.stories.unpublished = await Story.countDocuments({
      published: false,
    });
    ...
    allStories.forEach((story) => {
      ...
      diagnostic.stories.byAuthor[author] =
        (diagnostic.stories.byAuthor[author] || 0) + 1;
    });

    // Analyze by category
    allStories.forEach((story) => {
      const category = story.category || "Unknown";
      diagnostic.stories.byCategory[category] =
        (diagnostic.stories.byCategory[category] || 0) + 1;
    });
    ...
    if (allStories.length > 0) {
      diagnostic.stories.newestStory = {
        ...
        createdAt: allStories[0].createdAt
          ? allStories[0].createdAt.toISOString()
      ...
      diagnostic.stories.oldestStory = {
        ...
        createdAt: allStories[allStories.length - 1].createdAt
          ? allStories[allStories.length - 1].createdAt.toISOString()
          : new Date().toISOString(),
      };
    }
    ...
    if (diagnostic.stories.total < 10) {
      diagnostic.issues.push(
        `Only ${diagnostic.stories.total} stories found - this seems low for a production site`,
      );
    }
    ...
    if (diagnostic.stories.createdThisMonth === 0) {
      diagnostic.issues.push(
        "No stories created this month - potential data loss?",
      );
    }
    ...
    if (maxStoriesPerAuthor > diagnostic.stories.total * 0.8) {
      diagnostic.issues.push(
        "Most stories by single author - potential bulk deletion of other authors?",
      );
    }
    ...
    try {
      ...
      console.log(
        `[DATABASE DIAGNOSTIC] JSON serialization test passed (${testSerialization.length} chars)`,
      );
    } catch (serializationError) {
      console.error(
        `[DATABASE DIAGNOSTIC] JSON serialization failed:`,
        serializationError,
      );
      return res.status(500).json({
        success: false,
