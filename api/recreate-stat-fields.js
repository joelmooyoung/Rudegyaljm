...
async function removeProblematicFields() {
  ...
  console.log(`[RECREATE STATS] Removed problematic fields from ${removeResult.modifiedCount} stories`);
...
async function recreateCleanFields() {
  ...
  console.log(`[RECREATE STATS] Recreated clean fields in ${recreateResult.modifiedCount} stories`);
...
async function verifyFieldsWork() {
  ...
  return {
    ...
    operationsWork: {
      setWorked: afterSetObj.viewCount === 100 && afterSetObj.likeCount === 50 && afterSetObj.rating === 4.5,
      incrementWorked: afterIncrementObj.viewCount === 105 && afterIncrementObj.likeCount === 53 && afterIncrementObj.rating === 4.7
    }
  };
}
