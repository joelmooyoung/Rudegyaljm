...
export default async function handler(req, res) {
  ...
  return res.status(503).json({
    success: false,
    message:
      "Database seeding endpoint is DISABLED to prevent accidental data loss.",
    issue:
      "This endpoint was deleting real user accounts (including joelmooyoung@me.com) and replacing them with test users (admin@nocturne.com, premium@test.com, free@test.com).",
    impact:
      "Users logging in as their real accounts were seeing test user data instead.",
    solution:
      "Use development-specific seeding scripts that don't affect production data.",
  });
