  ...
  try {
    ...
    switch (req.method) {
      case "GET":
        ...
        const limit =
          parseInt(req.query.limit) || (includeUnpublished ? 20 : 50); // Smaller limit for admin
        const skip = (page - 1) * limit;

        console.log(
          `[STORIES API] Fetching page ${page}, limit ${limit}, skip ${skip}`,
        );
        ...
        console.log(
          `[STORIES API] Fast admin mode - using stored comment counts for ${stories.length} stories`,
        );
        ...
        if (includeUnpublished) {
          try {
            ...
            console.log(
              `[STORIES API] Page ${page} of ${Math.ceil(totalCount / limit)} (${skip + 1}-${skip + transformedStories.length} of ${totalCount})`,
            );
          } catch (countError) {
            console.warn(
              `[STORIES API] Failed to get total count:`,
              countError,
            );
          }
        }
        ...
        try {
          ...
          console.log(
            `[STORIES API] JSON serialization test passed (${testSerialization.length} chars)`,
          );
        } catch (serializationError) {
          console.error(
            `[STORIES API] JSON serialization failed:`,
            serializationError,
          );
          ...
          for (let i = 0; i < transformedStories.length; i++) {
            try {
              ...
              console.error(
                `[STORIES API] Story ${i} (${transformedStories[i]?.id}) failed serialization:`,
                storyError,
              );
          ...
          console.log(
            `[STORIES API] Removed problematic stories, returning ${transformedStories.length} stories`,
          );
        }
