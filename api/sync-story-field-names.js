    ...
    for (const story of stories) {
      ...
      if (storyObj.viewCount !== undefined && storyObj.views !== undefined) {
        if (storyObj.viewCount !== storyObj.views) {
          const maxViews = Math.max(storyObj.viewCount || 0, storyObj.views || 0);
          updateData.viewCount = maxViews;
          updateData.views = maxViews;
          needsUpdate = true;
        }
      } else if (storyObj.views !== undefined && storyObj.viewCount === undefined) {
        updateData.viewCount = storyObj.views;
        needsUpdate = true;
      } else if (storyObj.viewCount !== undefined && storyObj.views === undefined) {
        updateData.views = storyObj.viewCount;
        needsUpdate = true;
      }

      // Sync rating: use rating if it exists and is greater, otherwise use averageRating
      if (storyObj.rating !== undefined && storyObj.averageRating !== undefined) {
        if (storyObj.rating !== storyObj.averageRating) {
          const maxRating = Math.max(storyObj.rating || 0, storyObj.averageRating || 0);
          updateData.rating = maxRating;
          updateData.averageRating = maxRating;
          needsUpdate = true;
        }
      } else if (storyObj.averageRating !== undefined && storyObj.rating === undefined) {
        updateData.rating = storyObj.averageRating;
        needsUpdate = true;
      } else if (storyObj.rating !== undefined && storyObj.averageRating === undefined) {
