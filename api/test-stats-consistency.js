...
export default async function handler(req, res) {
  console.log(`[TEST STATS CONSISTENCY] ${req.method} /api/test-stats-consistency`);
  ...
  try {
    ...
    const storiesResponse = await fetch(`http://localhost:8080/api/test-stories-listing`);
    const storiesData = await storiesResponse.json();
    const amsterdamFromListing = storiesData.stories.find(s => s.id === testStoryId);

    // Test 2: Individual Story Stats API (used by story reader)
    const individualResponse = await fetch(`http://localhost:8080/api/stories/${testStoryId}/stats`);
    const individualData = await individualResponse.json();

    // Test 3: Admin Stats API (used by admin pages)
    const adminResponse = await fetch(`http://localhost:8080/api/admin/refresh-stats`);
    ...
    const fields = ['viewCount', 'likeCount', 'rating', 'ratingCount', 'commentCount'];
    ...
    for (const field of fields) {
      ...
      consistencyCheck[field] = {
        isConsistent: uniqueValues.length <= 1 || (uniqueValues.length === 2 && uniqueValues.includes(undefined)),
        values: values,
        uniqueValues: uniqueValues
      };
    }

    const overallConsistent = Object.values(consistencyCheck).every(check => check.isConsistent);

    console.log(`[TEST STATS CONSISTENCY] Overall consistency: ${overallConsistent}`);
    ...
    return res.status(200).json({
      ...
      summary: {
        ...
        mainStatsWorking: consistencyCheck.viewCount.isConsistent && consistencyCheck.rating.isConsistent,
      },
