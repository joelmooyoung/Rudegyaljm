...
export default function QueryOptimization({ onNavigateBack }: QueryOptimizationProps) {
        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex items-center justify-center py-12">
            <div className="flex items-center gap-3">
              ...
              <span className="text-lg">Running database query analysis...</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-2">
              Analyze and optimize database query performance for statistics endpoints
            </p>
          </div>
        ...
        {analysisData && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{analysisData.summary.totalQueries}</div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      <div className="flex justify-between items-center">
                        ...
                        <Badge variant="outline">{analysisData.overallAnalysis.totalQueries}</Badge>
                      </div>
                      <div className="flex justify-between items-center">
                        <span>Queries with Issues</span>
                        <Badge variant="secondary">{analysisData.overallAnalysis.queriesWithIssues}</Badge>
                      </div>
                      <div className="flex justify-between items-center">
                        <span>Critical Queries</span>
                        <Badge variant="destructive">{analysisData.overallAnalysis.criticalQueries}</Badge>
                      </div>
                      <div className="flex justify-between items-center">
                        <span>Health Score</span>
                        <Badge 
                          variant={analysisData.overallAnalysis.healthScore > 80 ? "default" : "destructive"}
                        >
          ...
          <TabsContent value="critical" className="space-y-4">
            {analysisData?.criticalIssues && analysisData.criticalIssues.length > 0 ? (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    These queries are performing full collection scans or have severe performance issues
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
                      ...
                      <Alert key={index} variant="destructive">
                        ...
                        <AlertDescription>
                          <div className="mt-2 space-y-2">
                            <p><strong>Issue:</strong> {issue.issue.message}</p>
                            <p><strong>Impact:</strong> {issue.issue.impact}</p>
          ...
          <TabsContent value="collections" className="space-y-4">
            {analysisData?.explainResults && (
              <div className="space-y-6">
                {Object.entries(analysisData.explainResults).map(([collection, queries]: [string, any]) => (
                  <Card key={collection}>
                    <CardHeader>
                      <CardTitle className="capitalize">{collection} Collection</CardTitle>
                      <CardDescription>
                    ...
                    <CardContent>
                      {queries.error ? (
                        ...
                        <Table>
                          ...
                          <TableBody>
                            {Object.values(queries).map((query: any, index: number) => (
                              query.queryName && (
                                <TableRow key={index}>
                                  <TableCell className="font-medium">{query.queryName}</TableCell>
                                  <TableCell>
                                    <Badge 
                                      variant={query.performance === 'good' ? 'default' : query.performance === 'critical' ? 'destructive' : 'secondary'}
                                      className={`${getPerformanceColor(query.performance)}`}
                                    >
                                      {query.performance}
                                    </Badge>
                                  </TableCell>
                                  <TableCell>{query.executionStats?.totalDocsExamined || 0}</TableCell>
                                  <TableCell>{query.executionStats?.totalDocsReturned || 0}</TableCell>
                                  <TableCell>
                                    {query.issues?.length > 0 ? (
                                      <div className="flex flex-col gap-1">
                                        {query.issues.map((issue: any, i: number) => (
                                          <Badge 
                                            key={i}
                                            variant="outline" 
                                  ...
                                  <TableCell>
                                    {query.indexesUsed?.length > 0 ? (
                                      query.indexesUsed.map((index: string, i: number) => (
          ...
          <TabsContent value="optimized" className="space-y-4">
            {optimizedQueries?.optimizedQueries && (
              <div className="space-y-6">
                {optimizedQueries.optimizedQueries.map((query: any, index: number) => (
                  <Card key={index}>
                    <CardHeader>
                      <CardTitle className="flex items-center gap-2">
                    ...
                    <CardContent>
                      ...
                      <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                          ...
                          <Badge variant="outline" className="ml-2">{query.requiredIndex}</Badge>
                        </div>
                        <div>
                          <strong>Expected Gain:</strong>
                          <Badge variant="default" className="ml-2">{query.performanceGain}</Badge>
                        </div>
                        <div>
                          <strong>Category:</strong>
                          <Badge variant="secondary" className="ml-2">{query.category}</Badge>
                        </div>
                      </div>
                      
                      <div className="mt-3">
                        <strong>Explanation:</strong>
                        <p className="text-sm text-muted-foreground mt-1">{query.explanation}</p>
                      </div>
                    </CardContent>
                  </Card>
                ))}
                ...
                {analysisData.recommendations.map((rec: any, index: number) => (
                  <Card key={index}>
                    <CardHeader>
                      ...
                      <CardDescription>
                        Priority: <Badge variant={rec.priority === 'critical' ? 'destructive' : rec.priority === 'high' ? 'default' : 'secondary'}>
                          {rec.priority}
                        </Badge>
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                      <div className="space-y-3">
                        <div>
                          <strong>Collection:</strong> <code>{rec.collection}</code>
                        </div>
                        {rec.index && (
                          <div>
                            <strong>Index:</strong> <code className="bg-muted px-2 py-1 rounded">{rec.index}</code>
                          </div>
                        )}
                        <div>
        ...
        {analysisData?.nextSteps && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>Recommended actions to improve query performance</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
                {analysisData.nextSteps.map((step: any, index: number) => (
                  <div key={index} className="flex items-start gap-3 p-3 border rounded">
                    <Badge variant="outline" className="mt-0.5">Step {step.step}</Badge>
                    ...
                    <Badge variant={step.priority === 'immediate' ? 'destructive' : 'default'}>
                      {step.priority}
                    </Badge>
                  </div>
                ))}
