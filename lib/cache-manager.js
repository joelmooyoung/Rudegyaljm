// Conditional Redis import - only import if Redis is available
let createClient = null;
try {
  const redisModule = await import('redis');
  createClient = redisModule.createClient;
} catch (error) {
  console.log('[CACHE MANAGER] Redis not installed, using in-memory cache only');
}
...
class CacheManager {
  constructor() {
    ...
    console.log(`[CACHE MANAGER] Initialized with ${CACHE_CONFIG.USE_REDIS ? 'Redis' : 'in-memory'} backend`);
  }
  ...
  async initRedis() {
    try {
      ...
      console.warn('[CACHE MANAGER] Redis connection failed, falling back to in-memory cache:', error);
      this.redisClient = null;
    }
  }
    ...
    try {
      if (this.redisClient) {
        ...
        if (keys.length > 0) {
          ...
          console.log(`[CACHE MANAGER] ✅ Invalidated ${keys.length} keys matching pattern: ${pattern}`);
        }
      } else {
        ...
        console.log(`[CACHE MANAGER] ✅ Invalidated ${invalidatedCount} keys matching pattern: ${pattern}`);
      }
    } catch (error) {
      console.error(`[CACHE MANAGER] Error invalidating pattern ${pattern}:`, error);
      this.cacheStats.errors++;
    }
  }
  ...
  cleanupExpiredMemoryEntries() {
    ...
    if (cleanedCount > 0) {
      console.log(`[CACHE MANAGER] 🧹 Cleaned up ${cleanedCount} expired memory entries`);
    }
  }
...
export const triggerUserCacheInvalidation = () => cacheManager.invalidateUserCache();
export const triggerStoryCacheInvalidation = () => cacheManager.invalidateStoryCache();
export const triggerEngagementCacheInvalidation = () => cacheManager.invalidateEngagementCache();
export const triggerStatsCacheInvalidation = () => cacheManager.invalidateStatsCache();
